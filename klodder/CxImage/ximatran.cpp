		...
		for(long y=0; y<newy; y++){
			...
			for(long x=0; x<newx; x++){
				...
				for(int m=-1; m<3; m++) {
					...
					for(int n=-1; n<3; n++) {
						...
						if (head.biClrUsed){
							...
							_iDst  = info.pImage + yy*info.dwEffWidth + xx*3;
							rgb.rgbBlue = *_iDst++;
							rgb.rgbGreen= *_iDst++;
							rgb.rgbRed  = *_iDst;
						}
				...
				if (head.biClrUsed)
					...
					_iDst = newImage.info.pImage + y*newImage.info.dwEffWidth + x*3;
					*_iDst++ = (BYTE)bb;
					*_iDst++ = (BYTE)gg;
					*_iDst   = (BYTE)rr;
				}
			...
			for(long y=0; y<newy; y++){
				...
				for(long x=0; x<newx; x++){
					...
					if (head.biClrUsed){
						...
						BYTE* _iDst;
						_iDst = info.pImage + ifY*info.dwEffWidth + ifX*3;
						rgb1.rgbBlue = *_iDst++;	rgb1.rgbGreen= *_iDst++;	rgb1.rgbRed =*_iDst;
						_iDst = info.pImage + ifY*info.dwEffWidth + ifX1*3;
						rgb2.rgbBlue = *_iDst++;	rgb2.rgbGreen= *_iDst++;	rgb2.rgbRed =*_iDst;
						_iDst = info.pImage + ifY1*info.dwEffWidth + ifX*3;
						rgb3.rgbBlue = *_iDst++;	rgb3.rgbGreen= *_iDst++;	rgb3.rgbRed =*_iDst;
						_iDst = info.pImage + ifY1*info.dwEffWidth + ifX1*3;
						rgb4.rgbBlue = *_iDst++;	rgb4.rgbGreen= *_iDst++;	rgb4.rgbRed =*_iDst;
					}
...
//				alphaPtr = newImage.AlphaGetPointer(0, dy++);
				alphaPtr = newImage.AlphaGetPointer(0, ey++);
#endif
