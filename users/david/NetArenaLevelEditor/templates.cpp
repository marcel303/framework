...
Template* Template::GetMirror()
{
    ...
    t->m_middle.m_pixmap = new QPixmap(m_middle.m_pixmap->transformed(QTransform().scale(-1, 1)));
    t->m_front.m_pixmap  = new QPixmap(m_front.m_pixmap->transformed(QTransform().scale(-1, 1)));

    int x, y;
    t->GetMaxXY(x, y);

    t->m_mec.CreateLayer(x, y);
    t->m_col.CreateLayer(x, y);

    for(int j = 0; j < y; j++)
        for(int i = 0; i < x; i++)
        {
            t->m_mec.m_grid[j][i] = m_mec.m_grid[y-j][x-i];
            t->m_col.m_grid[j][i] = m_mec.m_grid[y-j][x-i];
        }
...
void TemplateFolder::SetTempTemplate(Template* t)
{
    m_currentTemplate = t;
}
