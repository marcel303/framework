...
{
public:
	PixelShader(ID3D11Device * pDevice, const char * pFileName, const char * pEntryPoint, const char ** pMacroList)
		: m_pShader(0)
		, m_pBytes(0)
	{
		...
		V(D3DX11CompileFromFileA(pFileName, macroList, 0, pEntryPoint, "ps_4_0", flags, 0, 0, &m_pBytes, &pErrorBytes, 0));
		...
		V(pDevice->CreatePixelShader(m_pBytes->GetBufferPointer(), m_pBytes->GetBufferSize(), 0, &m_pShader));
	}
