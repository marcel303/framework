		...
		else if (classification1 == pcFront)
		{
			...
			*vertex = *(*i)->vertex[0];
			...
			if (classification2 == pcBack)
			{
				...
				Vector delta = (*i)->vertex[1]->position - (*i)->vertex[0]->position;
...
void Poly::CalculateExtents(Vector& outMin, Vector& outMax) const
{
	...
	for (std::list<Vertex*>::const_iterator i = cVertex.begin(); i != cVertex.end(); ++i)
	{
		...
		else
		{
			for (int j = 0; j < 3; ++j)
			{
				if ((*i)->position[j] < outMin[j])
				{
					outMin[j] = (*i)->position[j];
				}
				else if ((*i)->position[j] > outMax[j])
				{
					outMax[j] = (*i)->position[j];
				}
			}
		}
	}
...
Vector Poly::CalculateCenter() const
{
	...
	const Vector center = (aabbMin + aabbMax) / 2.0f;
	
	return center;
	
}

Vector Poly::CalculateVertexCenter() const
{
...
Vector Poly::GetVertexCenter() const
{
...
void Poly::Finalize()
{
	...
	vertexCenter = Vector(0.0f, 0.0f, 0.0f);
