 ...
 */
void Geo::Builder::MakeCube(Mesh& mesh)
{
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			...
			temp->position[0] = (float)vertex[face[i][j]][0];
			temp->position[1] = (float)vertex[face[i][j]][1];
			temp->position[2] = (float)vertex[face[i][j]][2];
