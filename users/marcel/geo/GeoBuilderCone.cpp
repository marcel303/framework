 ...
 */
void Geo::Builder::MakeCone(Mesh& mesh, int slices)
{
	...
	for (int i = 0; i < slices; ++i)
	{
		...
		Vertex* vertex1 = poly->Add(); vertex1->position = Vector(0.0f,       0.0f,       +1.0f);
		Vertex* vertex2 = poly->Add(); vertex2->position = Vector(sin(a + s), cos(a + s), -1.0f);
		Vertex* vertex3 = poly->Add(); vertex3->position = Vector(sin(a  ),   cos(a  ),   -1.0f);
