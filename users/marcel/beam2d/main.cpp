...
int main(int argc, char* argv[])
{
	...
	framework.init(VIEW_SX, VIEW_SY);
...
static void TestBeamTree()
{
	...
	for (;;)
	{
		...
		framework.beginDraw(0, 0, 0, 0);
		{
			...
			bouncer[0].Position[0] = mouse.x;
			bouncer[0].Position[1] = mouse.y;
			...
			{
				const Color color_visible(0.0f, 1.0f, 0.0f);
				const Color color_invisible(1.0f, 0.0f, 0.0f);
				...
				for (float x = 0.0f; x < VIEW_SX; x += step)
				{
					for (float y = 0.0f; y < VIEW_SY; y += step)
					{
						...
						if (isVisible)
						{
							...
							hqFillCircle(p[0], p[1], 1.f);
						}
						else
						{
							setColor(color_invisible);
							hqFillCircle(p[0], p[1], 1.f);
						}
					}
				}
