...
{
	return sqrtf(m_xyz * m_xyz + m_w * m_w);
}
...
void Quaternion::FromAxisAngle(Vector axis, float angle)
{
	...
	m_xyz[0] = axis[0] * sinf(angle / 2.0f);
	m_xyz[1] = axis[1] * sinf(angle / 2.0f);
	m_xyz[2] = axis[2] * sinf(angle / 2.0f);
...
void Quaternion::FromMatrix(const Matrix& matrix)
{
	...
	{
		const float s = 2.0f * sqrtf(1.0f + matrix(2, 2) - matrix(0, 0) - matrix(1, 1)); 
...
void Quaternion::ToAxisAngle(Vector& out_axis, float& out_angle) const
{
	...
	out_angle = acosf(temp.m_w) * 2.0f;
	
	const float sinAngle = sinf(out_angle / 2.0f);
...
Quaternion Quaternion::Slerp(const Quaternion& quaternion, float t) const
{
	...
	if (1.0f - fabs(dot) > 0.000001f)
	{
		...
		s1 = sinf((1.0f - t) * angle) / angleSin;
		s2 = sinf((0.0f + t) * angle) / angleSin;
	}
	else
	{
		s1 = 1.0f - t;
