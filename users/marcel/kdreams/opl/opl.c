...
void enable_operator_ex(Bitu regbase, op_type* op_pt, Bit32u act_type) {
	...
	case ARC_KON_BNUM: {
		...
		if (idx == ARC_PERC_MODE) {
			...
			if ((val&0x30) == 0x30) {		// BassDrum active
				enable_operator_ex(16,&op[6],OP_ACT_PERC);
				change_frequency(6,16,&op[6]);
				enable_operator_ex(16+3,&op[6+9],OP_ACT_PERC);
		...
		if (base<9) {
			...
			if (val&32) {
				...
				enable_operator_ex(modbase,&op[opbase],OP_ACT_NORMAL);		// modulator (if 2op)
				enable_operator_ex(modbase+3,&op[opbase+9],OP_ACT_NORMAL);	// carrier (if 2op)
#if defined(OPLTYPE_IS_OPL3)
				// for 4op channels all four operators are switched on
				if ((adlibreg[0x105]&1) && op[opbase].is_4op) {
					// turn on chan+3 operators as well
					enable_operator_ex(modbase+8,&op[opbase+3],OP_ACT_NORMAL);
					enable_operator_ex(modbase+3+8,&op[opbase+3+9],OP_ACT_NORMAL);
				}
#endif
			} else {
	...
	case ARC_WAVE_SEL+0x10: {
		...
		if ((num<6) && (base<22)) {
			...
			if (adlibreg[0x01]&0x20) {
				...
				wave_sel[base] = val&3;
				op_ptr = &op[regbase2modop[base]+((num<3) ? 0 : 9)];
...
static void clipit16(Bit32s ival, Bit16s* outval) {
	*outval = ival < -32768 ? -32768 : ival > 32767 ? 32767 : ival;
}
