...
struct OscMessageHistory
{
	struct Elem
	{
		...
		void updateSlow(const float dt)
		{
			...
			const float retain = std::pow(smooth, dt);
				...
				gxPushMatrix();
				{
					...
					for (auto & elemItr : history.elems)
					{
						...
						gxPushMatrix();
						{
							...
							setColor(colorWhite);
							drawRect(smoothX1 - x, smoothY1 - y, smoothX1 - x + smoothSx * elem.smooth, smoothY2 - y);
							
							setLumi(40);
							drawText(
								(smoothX1 + smoothX2) / 2.f - x,
								(smoothY1 + smoothY2) / 2.f - y,
								8, 0, 0, "%d%%", int(std::round(elem.smooth * 100.f)));
							...
							if (hoverSmooth)
							{
								if (mouse.isDown(BUTTON_LEFT))
								{
									SDL_LockMutex(s_mutex);
									{
										for (auto & realElemItr : s_oscMessageHistory.elems)
										{
											auto & realElem = realElemItr.second;
											
											if (realElem.address == elem.address)
											{
												const float t = (mouse.x - smoothX1) / float(smoothSx);
												
												realElem.smooth = saturate(t);
											}
										}
									}
									SDL_UnlockMutex(s_mutex);
