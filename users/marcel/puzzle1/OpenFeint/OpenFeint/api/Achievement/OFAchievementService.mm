...
@synthesize mCustomUrlWithSocialNotification;
...
- (void) onAchievementUnlocked:(OFPaginatedSeries*)page nextCall:(OFDelegateChained*)nextCall duringSync:(BOOL)duringSync fromBatch:(BOOL) fromBatch
{
	...
	if (achievementCnt == 1)
	{
        ...
        {
            if ([mCustomUrlWithSocialNotification length] > 0)
                [OFSocialNotificationService sendWithAchievement:unlockedAchievement withSocialNotificationUrl:mCustomUrlWithSocialNotification];
            else
...
{
	return [OFAchievementService unlockAchievement:achievementId onSuccess:OFDelegate() onFailure:OFDelegate()];
}
...
{
	NSString* lastLoggedInUser = [OpenFeint lastLoggedInUserId];
	return [OFAchievementService sendPendingAchievements:lastLoggedInUser syncOnly:NO onSuccess:onSuccess onFailure:onFailure];
}
