...
int main(int argc, char * argv[])
{
	...
	lgen::DoubleBufferedHeightfield heightfield;
	heightfield.setSize(512, 512);
	
	lgen::Generator_OffsetSquare generator;
	generator.generate(heightfield.get(), rand());
	...
	GxTextureId texture = createTexture(heightfield.get());
	...
	for (;;)
	{
		...
		if (mouse.wentDown(BUTTON_LEFT))
		{
			...
			freeTexture(texture);
			texture = createTexture(heightfield.get());
		}
