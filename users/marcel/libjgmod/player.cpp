...
void JGMOD_PLAYER::mod_interrupt()
{
    ...
    if (mi.tick >= mi.tempo)
	{
        ...
        for (int chn = 0; chn < of->no_chn; ++chn)
		{
            if (chn >= mi.max_chn)
                continue;
		...
		// if pattern break or position jump
        if (mi.new_pos)
		{
            mi.pos = mi.new_pos - 1;
            mi.trk = mi.new_trk - 1;
    ...
    if (mi.tick == 0)
	{
        for (int chn = 0; chn < of->no_chn; ++chn)
		{
            ...
            const NOTE_INFO * ni = get_note(of, mi.pat, mi.pos, chn);
        ...
        for (int chn = 0; chn < of->no_chn; ++chn)
		{
            ...
            const int sample_no = ni->sample - 1;
                ...
                || ni->command == XMEFFECT_5 || (ni->volume & 0xF0) == 0xF0))
			{
				...
				{
                    const SAMPLE_INFO & si = of->si[get_jgmod_sample_no(sample_no, ci[chn].note)];
            ...
            if ((ni->note > 0 || sample_no >= 0) &&
            	ni->command != PTEFFECT_3 &&
            	ni->command != PTEFFECT_5 &&
            	ni->command != XMEFFECT_5 &&
            	(ni->volume & 0xF0) != 0xF0)
			{
...
//-- updates the sound output ------------------------------------------------
    for (int chn=0; chn < of->no_chn; ++chn)
	{
        ...
        if (ci[chn].pan < 0)
            ci[chn].pan = 0;
        else if (ci[chn].pan > 256)
            ci[chn].pan = 256;

        if (ci[chn].temp_pan < 0)
            ci[chn].temp_pan = 0;
        else if (ci[chn].temp_pan > 256)
            ci[chn].temp_pan = 256;
        ...
        if (ci[chn].kick == true)
		{
            ...
            SAMPLE & s = of->s[ci[chn].sample];
            const SAMPLE_INFO & si = of->si[ci[chn].sample];
        ...
        {
            voiceApi->voice_set_frequency(
            	voice_table[chn],
            	period2pitch(ci[chn].temp_period) * pitch_ratio);
		}
