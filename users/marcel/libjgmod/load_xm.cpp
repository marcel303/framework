...
namespace jgmod
{
	...
	int get_xm_info (const char *filename, int start_offset, JGMOD_INFO *ji)
	{
		...
		if (start_offset == 0)
			...
			sprintf_s (ji->type_name, sizeof(ji->type_name), "XM");
			}
		else
			{
			ji->type = JGMOD_TYPE_UNREAL_XM;
			sprintf_s (ji->type_name, sizeof(ji->type_name), "Unreal XM (UMX)");
			}
	...
	//-- load all those samples --------------------------------------------------
		...
		j->si = (SAMPLE_INFO*)jgmod_calloc (sizeof(SAMPLE_INFO) * (j->no_sample+1));
		if (j->si == nullptr)
			{
			free (wv);
			jgmod_destroy (j);
		...
		j->s  = (SAMPLE*)jgmod_calloc (sizeof (SAMPLE) * (j->no_sample+1));
		if (j->s == nullptr)
			{
			free (wv);
			jgmod_destroy (j);
