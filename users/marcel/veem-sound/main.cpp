...
struct Mechanism
{
	...
	float xAngleSpeed = 0.f;
	float yAngleSpeed = 0.f;
	float zAngleSpeed = 0.f;
	...
	void evaluateMatrix(const int ringIndex, Mat4x4 & matrix, float & radius) const
	{
		...
		if (ringIndex >= 1)
		{
			matrix = matrix.RotateX(-xAngle * M_PI / 180.f);
			radius = kRadius2;
		}
		
		if (ringIndex >= 2)
		{
			matrix = matrix.RotateY(-yAngle * M_PI / 180.f);
			radius = kRadius3;
		}
		
		if (ringIndex >= 3)
		{
			matrix = matrix.RotateX(-zAngle * M_PI / 180.f);
			radius = kRadius4;
		}
	}
