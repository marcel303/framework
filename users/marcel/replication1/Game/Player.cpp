	...
	m_weaponLink("weapon", this), m_fort("fort", this)
{
	...
	AddParameter(Parameter(PARAM_INT32, "color", REP_ONCHANGE, COMPRESS_NONE, &m_color));
	AddParameter(m_weaponLink.GetParameter());
}
...
void Player::PostCreate()
{
	...
	m_control.RegisterParameters(this);
	...
	m_sndJump = RESMGR.GetSnd("sounds/jump.ogg");
	m_sndHurt = RESMGR.GetSnd("sounds/hurt.ogg");
...
void Player::UpdateLogic(float dt)
{
	...
	if (hit.get() && distance <= 5.0f)
	{
		// TODO: Revert.
		if (0 && hit->m_className == "Floor")
		{
			ReplicatedMessage(MSG_PLAYSOUND, SND_HURT);

			m_phyObject.m_velocity[0] = 0.0f;
			m_phyObject.m_velocity[1] = 0.0f;
			m_phyObject.m_velocity[2] = 0.0f;

			//m_control.m_controls.m_vAccel += 300.0f;
			//m_phyObject.m_force[1] += 300.0f * 50.0f;
			m_phyObject.m_force[1] += 300.0f * 25.0f;

			EntityParticles* particles = new EntityParticles(m_phyObject.m_position, 10, 1.0f);
...
void Player::OnAction(int actionID, float value)
{
	...
	case ControllerExample::ACTION_FIRE:
		...
		m_parameters["firing"]->Invalidate();
		break;
