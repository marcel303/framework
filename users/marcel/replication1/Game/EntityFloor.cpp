...
EntityFloor::EntityFloor(Vec3 position, Vec3 size, int orientation) : Entity()
{
	...
	AddParameter(Parameter(PARAM_INT8,    "o",  REP_ONCE, COMPRESS_NONE, &m_orientation));
...
void EntityFloor::PostCreate()
{
	...
	m_shader->m_vs = RESMGR.GetVS("shaders/floor_vs.cg");
	m_shader->m_ps = RESMGR.GetPS("shaders/floor_ps.cg");

	m_tex = RESMGR.GetTex("textures/floor.png");
}
...
void EntityFloor::OnSceneAdd(Scene* scene)
{
	...
	((CD::Cube*)m_cdCube.get())->Setup(- m_size / 2.0f, m_size / 2.0f);
	//((CD::Cube*)m_cdCube.get())->Setup(m_position - m_size / 2.0f, m_position + m_size / 2.0f);
