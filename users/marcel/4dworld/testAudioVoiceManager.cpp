...
struct Creature
{
	...
	void init()
	{
		...
		g_voiceMgr->allocVoice(voice, &sine, "creature", true, 0.f);
...
struct RicePaddy
{
		...
		, voice(nullptr)
	{
		...
		g_voiceMgr->allocVoice(voice, &source, "ricePaddy", true, 0.f);
	}
...
struct TestObject
{
		...
		, uiState()
	{
		...
		g_voiceMgr->allocVoice(voice, &sine, "testObject", true, 0.f);
...
void testAudioVoiceManager()
{
	...
	voiceMgr.allocVoice(wavefield1DVoice, &wavefield1D, "wavefield1D", true, 0.f);
